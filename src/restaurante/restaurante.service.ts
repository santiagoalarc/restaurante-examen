...
import { RestauranteEntity, TipoCocina } from './restaurante.entity/restaurante.entity';
...
export class RestauranteService {
    ...
    async create(restaurante: RestauranteEntity): Promise<RestauranteEntity> {
        ...
        if (!tipoCocina.includes(restaurante.tipoCocina)) {
            throw new BusinessLogicException(`La categoría '${restaurante.tipoCocina}' no es válida.`, BusinessError.BAD_REQUEST);
        }
    ...
    async findOne(id: string): Promise<RestauranteEntity> {
        ...
        if (!restaurante) {
            throw new BusinessLogicException(`El restaurante con id ${id} no fue encontrado`, BusinessError.NOT_FOUND);
        }
        return restaurante;
    }
